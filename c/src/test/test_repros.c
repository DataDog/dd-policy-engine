...
static size_t observed_value_count;
...
UTEST(repro_actions, later_success_does_not_erase_failure) {
  ...
  ASSERT_EQ(plcs_eval_ctx_register_action(successful_action, PLCS_ACTION_INJECT_ALLOW), PLCS_ESUCCESS);
...
UTEST(repro_actions, schema_maximum_value_count_is_accepted) {
  const dd_wls_ActionId_enum_t ids[] = {dd_wls_ActionId_INJECT_ALLOW};
  repro_buffer buffer = build_action_policy(ids, 1, (size_t)PLCS_ACTION_VALUES_MAX);
  ASSERT_TRUE(buffer.data != NULL);
  ...
  /* ACTION_VALUES_MAX is documented as the maximum, not the first rejected size. */
  ASSERT_EQ(result, PLCS_ESUCCESS);
  ASSERT_EQ(observed_value_count, (size_t)PLCS_ACTION_VALUES_MAX);
}
