...
plcs_evaluation_result string_evaluator_exact(const char *eval, const char *param) {
  ...
  return strncmp(eval, param, strlen(eval) + 1) == 0 ? PLCS_EVAL_RESULT_TRUE : PLCS_EVAL_RESULT_FALSE;
}
...
plcs_evaluation_result string_evaluator_suffix(const char *eval, const char *param) {
  ...
  if (eval_len > param_len) {
    return PLCS_EVAL_RESULT_FALSE;  // eval is longer than param, cannot be suffix
  }
...
plcs_evaluation_result string_evaluator_contains(const char *eval, const char *param) {
  ...
  if (eval_len > param_len) {
    return PLCS_EVAL_RESULT_FALSE;  // eval is longer than param, cannot be contained in
                                    // param
  }

  if (eval_len == 0) {
    return PLCS_EVAL_RESULT_TRUE;  // empty string is contained in any string
  }
...
plcs_evaluation_result plcs_default_string_evaluator(
  ...
  plcs_eval_ctx_set_str_eval_error(eval_id, PLCS_EUNKNOWN_CMP);
  return PLCS_EVAL_RESULT_ABSTAIN;
}
...
plcs_evaluation_result plcs_default_numeric_evaluator(
  ...
  switch (cmp) {
    case PLCS_NUM_CMP_EQ:
      return (policy == ctx) ? PLCS_EVAL_RESULT_TRUE : PLCS_EVAL_RESULT_FALSE;
      break;

    case PLCS_NUM_CMP_GT:
      return (policy > ctx) ? PLCS_EVAL_RESULT_TRUE : PLCS_EVAL_RESULT_FALSE;
      break;

    case PLCS_NUM_CMP_GTE:
      return (policy >= ctx) ? PLCS_EVAL_RESULT_TRUE : PLCS_EVAL_RESULT_FALSE;
      break;

    case PLCS_NUM_CMP_LT:
      return (policy < ctx) ? PLCS_EVAL_RESULT_TRUE : PLCS_EVAL_RESULT_FALSE;
      break;

    case PLCS_NUM_CMP_LTE:
      return (policy <= ctx) ? PLCS_EVAL_RESULT_TRUE : PLCS_EVAL_RESULT_FALSE;
      break;

    case PLCS_NUM_CMP_UNKNOWN:
    case PLCS_NUM_CMP__COUNT:
      // error we should not get here!
      return PLCS_EVAL_RESULT_ABSTAIN;
      break;
  }

  plcs_eval_ctx_set_num_eval_error(eval_id, PLCS_EUNKNOWN_CMP);
  return PLCS_EVAL_RESULT_ABSTAIN;
}
...
plcs_evaluation_result plcs_default_unumeric_evaluator(
  ...
  plcs_eval_ctx_set_unum_eval_error(eval_id, PLCS_EUNKNOWN_CMP);
  return PLCS_EVAL_RESULT_ABSTAIN;
}
